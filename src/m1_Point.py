...
class Point(object):
    ...
    def move_to(self, x, y):
        """Moves this point to new x y location"""
        self.x_s = self.x
        self.y_s = self.y
        self.x = x
        self.y = y
        self.count += 1
        self.distance = self.distance + math.sqrt(((self.x - self.x_s) ** 2) + ((self.y - self.y_s) ** 2))

    def move_by(self, x, y):
        """Adds dx and dy to x y location"""
        self.x_s = self.x
        self.y_s = self.y
        self.x = x + self.x
        self.y = y + self.y
    ...
    def get_distance_from_start(self):
        distance = math.sqrt(((self.x - self.x_i) ** 2) + ((self.y - self.y_i) ** 2))
        return distance
    ...
    def closer_to(self, p2, p3):
        distance_s2 = math.sqrt(((p2.x - self.x) ** 2) + ((p2.y - self.y) ** 2))
        distance_s3 = math.sqrt(((p3.x - self.x) ** 2) + ((p3.y - self.y) ** 2))
    ...
    def halfway_to(self, p2):
        midpoint = Point((p2.x + self.x) / 2, (p2.y + self.y) / 2)
        return midpoint
